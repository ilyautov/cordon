...
**A gateway for MCP hosts.** Claude Desktop, Cursor and any other MCP host get the same three axes without hooks at all: `cordon mcp -- npx server-x` is a stdio proxy that starts the server itself, cleans tool descriptions and results, and gates every `tools/call` — a refused call never reaches the server. Each server's tools are pinned on first sight. If a description or schema changes later (a rug pull), or a new tool appears, that tool is hidden from the model and refused until you run `cordon mcp approve`. The blind spots are named up front: the host's built-in tools go past MCP, the exposure mark is not lifted inside a session (there are no user turns over MCP, so the human's naming is written into the policy as `task:`), and one gateway process serves one upstream. The direction of failure is better than the hooks': a dead gateway is a dead server, not a silent pass. Configuration and the full list are in [docs/install-mcp.md](docs/install-mcp.md).

**A middleware for LangChain agents.** An agent built on `createAgent` (LangChain.js v1) gets the same three axes as a middleware in its own loop: `createCordonMiddleware(...)` feeds the user's message on `beforeModel` — the certificate, the user-named atoms and the lifting of the exposure mark work exactly as on the hooks — and gates every tool call on `wrapToolCall`: a refused call never runs and the model reads the reason as the tool's error output, a rewritten one runs with the untrusted fragment cut out of the arguments, and every result is observed and cleaned. `langchain` is a peer dependency your application already has; Cordon's bundle does not pull it in. The blind spots: a tool answering with a `Command` or a non-text block cannot be cleaned (the session is marked, and the next consequential call escalates), and the source-influence footer does not exist on this transport. The usage example and the full list are in [docs/install-langchain.md](docs/install-langchain.md).

**Memory that outlives the session.** The attack that waits is the one the exposure mark alone cannot see: an agent reads a page on Monday, a note from it lands in `CLAUDE.md`, and on Thursday a fresh session — one that read nothing untrusted at all — reloads the file and obeys it (the shape of Claws, arXiv:2607.05189, and MINJA, arXiv:2503.03704). When the session writing into memory carries untrusted content — the exposure mark, or a page or tool result from outside read in any earlier turn — the write is recorded in `~/.cordon/memory/`: a path, a source label and a time, never the content. Every later session then starts under the exposure mark, with the same escalation and the same exemption for destinations the user named, until the user reviews the file and writes `cordon: trust memory` on a line of its own. The ledger lives thirty days, `cordon doctor` lists what is in it, and the owner is told in the journal on the day of the write. `CLAUDE.md`, `AGENTS.md`, `GEMINI.md` and the other reloaded instruction files are known by name, as is Gemini CLI's `save_memory`; a Mem0 store or a team's own rules file is declared in the policy under `memory`. On the battery both memory scenarios walk through without the ledger and are stopped with it. On a live Claude Code 2.1.282 the whole cycle — the write recorded, a later session refused, the directive lifting it — is recorded in [docs/live-run.md](docs/live-run.md).
...
**Any new message from the user lifts the exposure mark, a bare "yes" included.** The argument for this is that the human has seen the turn and could step in. A page can use that: it makes the model ask "shall I send the report to the collector?" and turns the human's reflexive "yes" into the lift. The destination was in the question the human read, so this is consent Cordon takes at its word, the same as `cordon: trust memory`. Telling an informed "yes" from a reflexive one is a question about meaning, and no rule in the path can answer it. The certificate and host bounds still apply after the lift, and provenance still refuses a destination the page wrote verbatim.
