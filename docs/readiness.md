...
| A read into a resource the user never named, led there by content | GitHub MCP "toxic agent flow" (Invariant Labs), Progent-style resource policies | after an untrusted read, a `resource`-role argument the user did not say escalates, reads included | `tests/gate/gate.test.ts`, battery scenario `resource/private-repo-from-public-issue` |
| Argument roles and a task mandate | Progent, CaMeL capabilities | `arguments` declares destination, resource and content fields; `destinations` lists what an autonomous task may reach; a name exempts only a destination field and never exec | `tests/gate/gate.test.ts`, `tests/policy/load.test.ts` |
| Agent configuration written by an injection | CVE-2025-53773 (Copilot), CVE-2025-54135 (Cursor) fixes in the IDEs themselves | after an untrusted read, a write to `.vscode/*.json`, `.mcp.json` and kin escalates; `audit` reports autoApprove, folder-open tasks, project-enabled MCP servers and a redirected model endpoint | `tests/gate/gate.test.ts`, `tests/audit/audit.test.ts` |
| Markdown image and link exfiltration in the answer | Microsoft's EchoLeak fix, GitHub's CamoLeak fix (disabling images) | after an untrusted read, LangChain cuts images and data-carrying links from the answer; the hooks warn in the footer | `tests/output/egress.test.ts`, `tests/adapters/langchain/middleware.test.ts` |
| An approval where no one can be asked | APPA-style hash-bound approvals | on the MCP gateway and LangChain, interactive mode's question names `cordon approve <id>`, bound to the exact call, used once, void after an hour | `tests/session/approvals.test.ts`, `tests/cordon-approvals.test.ts` |
...
- **Approvals that outlive one call.** In interactive mode a hook harness asks the human on every escalated call. Where nobody can be asked, Cordon keeps an approval only for one exact call, once, for an hour. An approval for a tool, a host or a session is state an attacker can aim at (see "Things that look like improvements" in [AGENTS.md](../AGENTS.md)).
